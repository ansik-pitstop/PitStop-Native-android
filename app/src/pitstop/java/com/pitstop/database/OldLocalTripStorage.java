...
public class OldLocalTripStorage {
   // public static Semaphore semaphore = new Semaphore(1);
    ...
    public static final String CREATE_TABLE_APPOINTMENT = "CREATE TABLE IF NOT EXISTS "
            + TABLES.TRIP_OLD.TABLE_NAME + "("
            + TABLES.COMMON.KEY_ID + " INTEGER PRIMARY KEY,"
            + TABLES.TRIP_OLD.KEY_START + " TEXT, "
            + TABLES.TRIP_OLD.KEY_END + " TEXT, "
            + TABLES.TRIP_OLD.KEY_START_ADDRESS + " TEXT, "
            + TABLES.TRIP_OLD.KEY_END_ADDRESS + " TEXT, "
            + TABLES.TRIP_OLD.KEY_TOTAL_DISTANCE + " DOUBLE, "
            + TABLES.TRIP_OLD.KEY_PATH + " TEXT, "
            + TABLES.COMMON.KEY_OBJECT_ID + " INTEGER, "
            + TABLES.COMMON.KEY_CREATED_AT + " DATETIME" + ")";

    private LocalDatabaseHelper databaseHelper;

    public OldLocalTripStorage(Context context) {
    ...
    private Trip cursorToTrip(Cursor c) {
        Trip trip = new Trip();
        trip.setStart(gson.fromJson(c.getString(c.getColumnIndex(TABLES.TRIP_OLD.KEY_START)),TripLocation.class));
        trip.setEnd(gson.fromJson(c.getString(c.getColumnIndex(TABLES.TRIP_OLD.KEY_END)),TripLocation.class));
        trip.setStartAddress(c.getString(c.getColumnIndex(TABLES.TRIP_OLD.KEY_START_ADDRESS)));
        trip.setEndAddress(c.getString(c.getColumnIndex(TABLES.TRIP_OLD.KEY_END_ADDRESS)));
        trip.setTotalDistance(c.getDouble(c.getColumnIndex(TABLES.TRIP_OLD.KEY_TOTAL_DISTANCE)));
    ...
    private Trip cursorToTripWithPath(Cursor c) {
        ...
        trip.setEndAddress(c.getString(c.getColumnIndex(TABLES.TRIP_OLD.KEY_END_ADDRESS)));
        trip.setTotalDistance(c.getDouble(c.getColumnIndex(TABLES.TRIP_OLD.KEY_TOTAL_DISTANCE)));
        trip.setPath((List<TripLocation>)gson.fromJson(c.getString(c.getColumnIndex(TABLES.TRIP_OLD.KEY_PATH)),locListType));
        return trip;
    }
    ...
    private ContentValues tripObjectToContentValues(Trip trip) {
        ...
        values.put(TABLES.TRIP_OLD.KEY_START,jsonData);
        jsonData = gson.toJson(trip.getEnd());
        values.put(TABLES.TRIP_OLD.KEY_END,jsonData);
        values.put(TABLES.TRIP_OLD.KEY_START_ADDRESS,trip.getStartAddress());
        values.put(TABLES.TRIP_OLD.KEY_END_ADDRESS,trip.getEndAddress());
        values.put(TABLES.TRIP_OLD.KEY_TOTAL_DISTANCE,trip.getTotalDistance());
        jsonData = gson.toJson(trip.getPath());
        values.put(TABLES.TRIP_OLD.KEY_PATH,jsonData);
