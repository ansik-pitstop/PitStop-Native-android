...
public class MaterialShowcaseSequence implements IDetachedListener {
    ...
    private MaterialShowcaseView currentItem;
    ...
    private void showNextItem() {
        ...
        if (mShowcaseQueue.size() > 0 && !mActivity.isFinishing()) {
            currentItem = mShowcaseQueue.remove();
            currentItem.setDetachedListener(this);
            currentItem.show(mActivity);
            if (mOnItemShownListener != null) {
                mOnItemShownListener.onShow(currentItem, mSequencePosition);
            }
        } else {
            /**
